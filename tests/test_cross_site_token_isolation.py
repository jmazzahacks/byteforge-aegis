...
def two_sites(clean_database):
    ...
    site_a = db_manager.create_site(Site(
        uuid=generate_uuid7(), name="Site A", domain="a.example.com",
    ...
    site_b = db_manager.create_site(Site(
        uuid=generate_uuid7(), name="Site B", domain="b.example.com",
    ...
    user_b = db_manager.create_user(User(
        uuid=generate_uuid7(), site_uuid=site_b.uuid, email="victim@b.example.com",
...
def test_verify_email_rejects_token_from_another_site(two_sites):
    ...
    with pytest.raises(ValueError, match="Invalid or expired"):
        auth_service.verify_email(token_obj.token, site_uuid=site_a.uuid)

    # Sanity: the same call with the correct site_id succeeds.
    result = auth_service.verify_email(token_obj.token, site_uuid=site_b.uuid)
    assert result.user.uuid == user_b.uuid
...
def test_check_verification_token_rejects_token_from_another_site(two_sites):
    ...
    with pytest.raises(ValueError, match="Invalid or expired"):
        auth_service.check_verification_token(token_obj.token, site_uuid=site_a.uuid)

    status = auth_service.check_verification_token(token_obj.token, site_uuid=site_b.uuid)
...
def test_reset_password_rejects_token_from_another_site(two_sites):
    ...
    with pytest.raises(ValueError, match="Invalid or expired"):
        auth_service.reset_password(token_obj.token, site_uuid=site_a.uuid, new_password="newpass99")
